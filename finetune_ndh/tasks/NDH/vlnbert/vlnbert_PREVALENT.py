...
class VLNBert(BertPreTrainedModel):
    ...
    def forward(self, mode, input_ids, token_type_ids=None,
        ...
        #set_trace()
        attention_mask = lang_mask #8,80

        if token_type_ids is None:
            token_type_ids = torch.zeros_like(input_ids)

        extended_attention_mask = attention_mask.unsqueeze(1).unsqueeze(2) #[bs, 1, 1, 80]
        ...
        if mode == 'language':
            ...
            for layer_module in self.lalayer:
                temp_output = layer_module(text_embeds, extended_attention_mask) #bs, 80, 768
                text_embeds = temp_output[0]

            sequence_output = text_embeds
            pooled_output = self.pooler(sequence_output) #bs, 768

            return pooled_output, sequence_output

        elif mode == 'visual':
            ''' LXMERT visual branch (no language processing during navigation) '''
            text_embeds = input_ids  #this is state
            ...
            for tdx, layer_module in enumerate(self.addlayer): #tdx is order 0,1,2
            ...
            return pooled_output, visual_action_scores, attended_language, attended_visual, lang_output
