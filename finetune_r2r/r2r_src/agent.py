...
class BaseAgent(object):
    ...
    def test_submit(self, iters=None, **kwargs):
        self.env.reset_epoch(shuffle=(iters is not None))   # If iters is not none, shuffle the env batch
        self.losses = []
        self.results = {}
        # We rely on env showing the entire batch before repeating anything
        ...
        if iters is not None:
            # For each time, it will run the first 'iters' iterations. (It was shuffled before)
            for i in range(iters):
                for traj in self.rollout_submit(**kwargs):
                    self.loss = 0
                    self.results[traj['instr_id']] = traj['path']
        else:   # Do a full round
            while True:
...
class Seq2SeqAgent(BaseAgent):
    ...
    def __init__(self, env, results_path, tok, episode_len=20):
        ...
        # Optimizers
        #set_trace()
        self.vln_bert_optimizer = args.optimizer(self.vln_bert.parameters(), lr=args.lr, weight_decay=0.01)
        self.critic_optimizer = args.optimizer(self.critic.parameters(), lr=args.lr, weight_decay=0.01)
    ...
    def rollout_submit(self, train_ml=None, train_rl=True, reset=True):
        """
        :param train_ml:    The weight to train with maximum likelihood
        :param train_rl:    whether use RL in training
        :param reset:       Reset the environment
...
#            last_ndtw[i] = self.ndtw_criterion[ob['scan']](path_act, ob['gt_path'], metric='ndtw')
    ...
    def test(self, use_dropout=False, feedback='argmax', allow_cheat=False, iters=None, env_mode = None):
        ...
        #set_trace()
        if env_mode == 'test':
            super(Seq2SeqAgent, self).test_submit(iters)
        else:
